...
class PowerWindowTest2 {
    ...
    void powerWindowAdvanceByCanAdvanceOverSeveralBatches() throws IOException {
        ...
        var sampleBytes = Base64.getDecoder().decode(PowerComputerTest2.SAMPLES_BIN_BASE64);
        ...
        try (var s = new ByteArrayInputStream(bytes)) {
            ...
            var expected = Arrays.copyOfRange(PowerComputerTest2.POWER_SAMPLES, 0, STANDARD_WINDOW_SIZE);
    ...
    void powerWindowGetWorksOnGivenSamples() throws IOException {
        try (var sampleStream = PowerComputerTest2.getSamplesStream()) {
            var windowSize = 100;
            var w = new PowerWindow(sampleStream, windowSize);
            for (var offset = 0; offset < 100; offset += 1) {
                var expected = Arrays.copyOfRange(PowerComputerTest2.POWER_SAMPLES, offset, offset + windowSize);
    ...
    void powerWindowGetWorksAcrossBatches() throws IOException {
        ...
        try (var s = new ByteArrayInputStream(bytes)) {
            ...
            for (int i = 0; i < STANDARD_WINDOW_SIZE; i += 1)
                assertEquals(PowerComputerTest2.POWER_SAMPLES[i], w.get(i));
        }
    }
