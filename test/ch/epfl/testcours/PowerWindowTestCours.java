...
import ch.epfl.javions.demodulation.PowerComputerTest;
...
class PowerWindowTestCours {
    ...
    void powerWindowAdvanceByCanAdvanceOverSeveralBatches() throws IOException {
        ...
        var sampleBytes = Base64.getDecoder().decode(PowerComputerTestCours.SAMPLES_BIN_BASE64);
        ...
        try (var s = new ByteArrayInputStream(bytes)) {
            ...
            var expected = Arrays.copyOfRange(PowerComputerTestCours.POWER_SAMPLES, 0, STANDARD_WINDOW_SIZE);
    ...
    void powerWindowGetWorksOnGivenSamples() throws IOException {
        try (var sampleStream = PowerComputerTestCours.getSamplesStream()) {
            var windowSize = 100;
            var w = new PowerWindow(sampleStream, windowSize);
            for (var offset = 0; offset < 100; offset += 1) {
                var expected = Arrays.copyOfRange(PowerComputerTestCours.POWER_SAMPLES, offset, offset + windowSize);
    ...
    void powerWindowGetWorksAcrossBatches() throws IOException {
        ...
        try (var s = new ByteArrayInputStream(bytes)) {
            ...
            for (int i = 0; i < STANDARD_WINDOW_SIZE; i += 1)
                assertEquals(PowerComputerTestCours.POWER_SAMPLES[i], w.get(i));
        }
    }
